...
class B2CSalesOrder(Base):
    ...
    gross_margin_amount: Mapped[Decimal | None] = mapped_column(Numeric(12, 4), nullable=True)
    gross_margin_percent: Mapped[Decimal | None] = mapped_column(Numeric(12, 4), nullable=True)
    loyverse_receipt_id: Mapped[str | None] = mapped_column(String(100), nullable=True, index=True)
    loyverse_receipt_number: Mapped[str | None] = mapped_column(String(100), nullable=True, index=True)
    loyverse_receipt_date: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    loyverse_receipt_status: Mapped[str | None] = mapped_column(String(50), nullable=True)
    loyverse_payment_type_id_snapshot: Mapped[str | None] = mapped_column(String(100), nullable=True)
    loyverse_payment_type_name_snapshot: Mapped[str | None] = mapped_column(String(255), nullable=True)
    loyverse_source: Mapped[str | None] = mapped_column(String(50), nullable=True)
    loyverse_imported_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    loyverse_raw_payload_summary: Mapped[str | None] = mapped_column(Text, nullable=True)
    observations: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(
        DateTime,
